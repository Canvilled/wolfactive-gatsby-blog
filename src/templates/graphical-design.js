...
export default ({data,pageContext}) =>{
  ...
  const nextPage = `/graphical-design/trang-${currentPage +1}`;
  const prevPage = currentPage - 1 === 1 ? '/' : `/graphical-design/trang-${String(currentPage-1)}`;
  const isLastPage = currentPage === numberOfPages;
  return (
    <PageTransition>
      <Layout>
        <SEO title="Web Development"/>
        <h2 className="title--section text--center my-15">Chuyên mục: DESIGN</h2>
           ...
           <div className="pagination">
              <div className="pagination__container">
                ...
                {((currentPage === 1 )&&(numberOfPages !== 1))&& (
                  <>
                  <div className="pagination__item">
                    <Link to="graphical-design" rel="current" className="pagination__number active">
                      {currentPage }
                    </Link>
                  </div>
                  <div className="pagination__item">
